...
class Data:
    ...
    no_found_for_search = [
        'length', 'python', 'pytest', 'selenium', 'requests', 'faker']

    def random_book_id(self) -> str:
        rand_int = random.randint(0, len(self.books)-1)
        book_id = self.books[rand_int]['isbn']

        return book_id

    def get_book_by_book_id(
            self,
            book_id: str,
    ) -> dict:

        for element in self.books:
            if element['isbn'] == book_id:
                result = element
                break
