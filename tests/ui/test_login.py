...
def test_new_user_button(ui_login):
    ui_login.press_register_button()
...
def test_unsuccess_login(ui_login):
    username = faker.full_name()
    password = faker.password()

    assert ui_login.check_is_element_is_invisible('error_message')

    ui_login.fill_username_field(username)
    ui_login.fill_password_field(password)
    ui_login.press_login_button()

    assert ui_login.check_is_element_is_invisible('error_message') is False
